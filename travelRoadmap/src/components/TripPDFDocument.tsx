...
function drawRow(doc: jsPDF, label: string, value: string, y: number): number {
    ...
    const lines = doc.splitTextToSize(value, doc.internal.pageSize.getWidth() - 85);
    doc.text(lines, 65, y);
...
const TRANSPORT_LABELS: Record<string, string> = {
    car: 'Car (Rs. 800/day)',
    bike: 'Bike (Rs. 300/day)',
    bus: 'Bus (Rs. 150/day)',
    train: 'Train (Rs. 200/day)',
    cab: 'Cab/Taxi (Rs. 600/day)',
};
...
async function buildTripPDFDoc(data: TripDataForPDF): Promise<jsPDF> {
    ...
    y = drawRow(doc, 'Budget', `Rs. ${data.budget.toLocaleString()} per person`, y);
    ...
    if (data.stayType === 'hotel') {
        y = drawRow(doc, 'Price/Night', `Rs. ${data.hotelPrice.toLocaleString()}`, y);
        y = drawRow(doc, 'Total Stay', `Rs. ${(data.hotelPrice * data.days).toLocaleString()} (${data.days} nights)`, y);
    }
    y += 4;
    ...
    if (places.length === 0) {
        ...
        doc.text('No places selected for this itinerary.', 20, y);
        y += 10;
    } else {
        ...
        for (let dayIdx = 0; dayIdx < data.days; dayIdx++) {
            ...
            dayPlaces.forEach((place, i) => {
                ...
                // Truncate place name if too long
                const maxNameLen = 40;
                ...
                doc.text(displayName, 48, y);
                doc.setFont('helvetica', 'normal');
                doc.setTextColor(120, 120, 120);
                
                const entryFee = place.entryFee !== undefined ? `Rs. ${place.entryFee}` : 'Free';
                const visitTime = place.visitTime || `${place.visitMinutes || 60} mins`;
                const category = place.category || 'Attraction';
                
                doc.text(`${category} · ${visitTime} · ${entryFee}`, 48, y + 4);
                y += 10;
            });
            y += 2;
        }
    ...
    // Calculate actual entry fees from places if available
    let entryFees = 0;
    if (places && places.length > 0) {
        entryFees = places.reduce((sum, p) => sum + (p.entryFee || 0), 0);
    } else {
         entryFees = 80 * data.days; // fallback estimate
    }
    ...
    y = drawRow(doc, 'Accommodation', data.stayType === 'hotel' ? `Rs. ${accommodation.toLocaleString()}` : 'Free (Local Host)', y);
    y = drawRow(doc, 'Transport', `Rs. ${transport.toLocaleString()}`, y);
    y = drawRow(doc, 'Entry Fees', `Rs. ${entryFees.toLocaleString()}`, y);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(24, 70, 90);
    doc.text(`Total per Person:  Rs. ${total.toLocaleString()}`, 20, y);
    y += 6;
    const remaining = data.budget - total;
    doc.setFontSize(10);
    doc.setTextColor(remaining >= 0 ? 34 : 220, remaining >= 0 ? 139 : 38, remaining >= 0 ? 34 : 38);
    doc.text(remaining >= 0 ? `Budget Remaining: Rs. ${remaining.toLocaleString()}` : `Over Budget by: Rs. ${Math.abs(remaining).toLocaleString()}`, 20, y);
    y += 10;
