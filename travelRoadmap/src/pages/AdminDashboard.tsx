...
import { getAllHosts, getDestinationStats, getPendingRegistrations, approveHost, rejectHost } from '../services/adminService';
import { DBHostRegistration } from '../services/supabaseClient';
...
export default function AdminDashboard() {
    ...
    const [pendingHosts, setPendingHosts] = useState<DBHostRegistration[]>([]);
    ...
    const handleApprove = async (id: number) => {
        setActionLoading(id);
        const success = await approveHost(id);
        if (success) {
    ...
    const handleReject = async () => {
        ...
        const success = await rejectHost(rejectModal.id, rejectReason);
        if (success) {
