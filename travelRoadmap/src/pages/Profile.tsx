...
export default function Profile() {
    ...
    const [safetyContacts, setSafetyContacts] = useState<AppSafetyContact[]>([]);
    ...
    const handleAddContact = async () => {
        ...
        const { success, contact } = await addSafetyContact(newContactName, newContactPhone);
        if (success && contact) {
            setSafetyContacts([...safetyContacts, contact]);
            setNewContactName('');
            setNewContactPhone('');
    ...
    const handleDeleteContact = async (id: number) => {
        if (id === 0) return; // don't delete defaults
        if (window.confirm('Remove this safety contact?')) {
            const success = await deleteSafetyContact(id);
            if (success) {
                                ...
                                {safetyContacts.map((contact, i) => (
                                    <div key={i} className="flex items-center justify-between p-3 bg-white border border-gray-100 rounded-xl hover:shadow-sm transition-shadow">
                                        <div>
                                            ...
                                            <p className="text-gray-500 text-xs flex items-center gap-1 mt-0.5"><Phone size={12} /> {contact.number}</p>
                                        </div>
                                        {contact.id !== 0 && (
                                            <button onClick={() => handleDeleteContact(contact.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                                <X size={16} />
                                            </button>
                                        )}
                                    </div>
                                ...
                                {isAddingContact ? (
                                    <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 mt-4">
                                        <div className="flex flex-col gap-3">
                                            <input
                                                type="text"
                                                placeholder="Contact Name (e.g., Mom, Friend)"
                                            ...
                                            <div className="flex gap-2 justify-end mt-2">
                                                <button onClick={() => setIsAddingContact(false)} className="px-4 py-2 text-sm text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">Cancel</button>
                                                <button onClick={handleAddContact} className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm">Save Contact</button>
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                                    <button onClick={() => setIsAddingContact(true)} className="w-full p-3 border-2 border-dashed border-gray-300 rounded-xl text-gray-500 hover:border-green-400 hover:text-green-600 transition-all text-sm font-medium mt-2">
