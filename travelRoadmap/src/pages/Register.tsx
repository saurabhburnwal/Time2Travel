...
export default function Register() {
    ...
    const { register, isLoggedIn, isAdmin, loading: authLoading } = useAuth();
    const navigate = useNavigate();

    // If already logged in, redirect away from register page
    // But skip if we're in the middle of a host registration redirect
    if (!authLoading && isLoggedIn && !sessionStorage.getItem('t2t_host_redirect')) {
        return <Navigate to={isAdmin ? '/admin' : '/plan'} replace />;
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Set flag before register so the isLoggedIn guard doesn't intercept
        if (form.role === 'host') {
            sessionStorage.setItem('t2t_host_redirect', 'true');
        }
        const { success, error, role } = await register({
            name: form.name,
            email: form.email,
            phone: form.phone,
            gender: form.gender,
        ...
        if (success) {
            ...
            const actualRole = role || form.role;
            if (actualRole === 'host') {
                navigate('/host-register', { replace: true });
                // Clean up flag after navigation
                setTimeout(() => sessionStorage.removeItem('t2t_host_redirect'), 100);
            } else {
                sessionStorage.removeItem('t2t_host_redirect');
