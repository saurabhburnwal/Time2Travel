...
import { getHosts, AppHost } from '../services/hostsService';
import { AppHotel } from '../services/supabaseClient';
...
export default function StaySelection() {
    ...
    const [hotels, setHotels] = useState<AppHotel[]>([]);
    const [hosts, setHosts] = useState<AppHost[]>([]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const [h, hostList] = await Promise.all([
                getHotels(trip.destination),
                getHosts(trip.destination),
            ]);
            setHotels(h);
            setHosts(hostList);
            setLoading(false);
                            ...
                            <div className="grid md:grid-cols-2 gap-5">
                                {hotels.map((hotel, i) => (
                                    ...
                                    >
                                        ...
                                        <p className="text-sm text-gray-500 flex items-center gap-1 mt-1"><MapPin size={14} /> {trip.destination}</p>
