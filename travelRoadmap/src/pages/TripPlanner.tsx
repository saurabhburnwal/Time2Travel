...
import { getStates, getDestinations } from '../services/destinationsService';
import { getTravelTypes, getGroupTypes, AppTravelType, AppGroupType } from '../services/travelTypesService';
...
export default function TripPlanner() {
    ...
    const [travelTypes, setTravelTypes] = useState<AppTravelType[]>([]);
    const [groupTypes, setGroupTypes] = useState<AppGroupType[]>([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        (async () => {
            setLoading(true);
            const [s, tt, gt] = await Promise.all([getStates(), getTravelTypes(), getGroupTypes()]);
            setStates(s);
            setTravelTypes(tt);
            setGroupTypes(gt);
            setLoading(false);
    ...
    useEffect(() => {
        ...
        (async () => {
            const d = await getDestinations(trip.state);
            setDestinations(d);
        })();
    }, [trip.state]);
            ...
            <div className="section-container max-w-5xl relative z-10">
                ...
                <div className="grid grid-cols-4 gap-3 mb-10">
                    {DESTINATION_IMAGES.map((img, i) => (
                        <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }} className="image-card h-24 md:h-32">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="feature-card">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {travelTypes.map((type, i) => (
                                    <button key={type.id} onClick={() => updateTrip({ travelType: type.name })}
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="feature-card">
                            ...
                            <div className="grid md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select value={trip.state} onChange={e => { updateTrip({ state: e.target.value, destination: '' }); }} className="select-field mt-2">
                                ...
                                <div>
                                    ...
                                    <select value={trip.destination} onChange={e => updateTrip({ destination: e.target.value })} className="select-field mt-2" disabled={!trip.state}>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="feature-card">
                            ...
                            <div className="grid md:grid-cols-2 gap-8">
                                <div>
                                    ...
                                    <div className="mt-4">
                                        <input type="range" min={2000} max={10000} step={500} value={trip.budget} onChange={e => updateTrip({ budget: Number(e.target.value) })} className="w-full" />
                                ...
                                <div>
                                    ...
                                    <div className="mt-4">
                                        <input type="range" min={2} max={7} step={1} value={trip.days} onChange={e => updateTrip({ days: Number(e.target.value) })} className="w-full" />
                                        <div className="flex justify-between mt-2">
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="feature-card">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {groupTypes.map(g => (
                                    <button key={g.id} onClick={() => updateTrip({ groupType: g.name })}
                        ...
                        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 }} className="travel-strip text-white">
                            <div className="relative z-10">
                                ...
                                <div className="space-y-3 text-sm">
                                    <div className="flex items-center justify-between"><span className="text-gray-300">State</span><span className="font-semibold text-ocean-200">{trip.state || '—'}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-gray-300">Destination</span><span className="font-semibold text-ocean-200">{trip.destination || '—'}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-gray-300">Travel Type</span><span className="font-semibold text-ocean-200">{trip.travelType || '—'}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-gray-300">Budget</span><span className="font-semibold text-ocean-200">Rs. {trip.budget.toLocaleString()}</span></div>
                                    <div className="flex items-center justify-between"><span className="text-gray-300">Duration</span><span className="font-semibold text-ocean-200">{trip.days} days</span></div>
                                    <div className="flex items-center justify-between"><span className="text-gray-300">Group</span><span className="font-semibold text-ocean-200">{trip.groupType || '—'}</span></div>
                                </div>
                                <motion.button onClick={handleContinue} disabled={!canContinue}
                                    whileHover={canContinue ? { scale: 1.03 } : {}} whileTap={canContinue ? { scale: 0.97 } : {}}
